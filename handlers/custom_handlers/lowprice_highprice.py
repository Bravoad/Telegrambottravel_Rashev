...
from keyboards.reply.keyboards import keyboard_commands
from loader import bot, logger, exception_handler
from database.database import user, DataBaseModel, Hotel
...
from keyboards import keyboards_text, calendar
...
def hotel_template(call: CallbackQuery, currency: str, days: int, hotel: Dict) -> Optional[str]:
    ...
    try:
        ...
        link = config.URL_HOTEL.format(hotel['id'])
        if currency == 'USD':
