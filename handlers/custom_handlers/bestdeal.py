...
def bestdeal_logic(call: CallbackQuery, result_hotels: List[Dict], result: List) -> Union[List[Dict], bool]:
    ...
    if len(result) < user.user.count_hotel + 5:
        config_data.settings.QUERY_BESTDEAL['pageNumber'] = int(config_data.settings.QUERY_BESTDEAL['pageNumber']) + 1
        if config_data.settings.QUERY_BESTDEAL['pageNumber'] == 4:
            return False
        else:
            bestdeal_additional_request(call, result)
    elif len(result) >= user.user.count_hotel + 5:
        config_data.settings.QUERY_BESTDEAL['pageNumber'] = 1
        return result
